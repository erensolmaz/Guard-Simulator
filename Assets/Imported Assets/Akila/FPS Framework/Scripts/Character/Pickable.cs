...
namespace Akila.FPSFramework
{
    ...
    public class Pickable : Item, IInteractable
    {
        ...
        public void Interact(InteractionsManager source)
        {
            ...
            if (isActive)
            {
                if (interactSound != null)
                    source.interactAudio?.Play(true, interactSound);
                else if (source.defaultInteractAudio)
                    source.interactAudio?.Play(true, source.defaultInteractAudio.audioClip);
            }
        ...
        protected virtual void DoItemPickup(IInventory inventory)
        {
            ...
            inventory.items = inventory.gameObject.gameObject.GetComponentsInChildren<InventoryItem>().ToList();
